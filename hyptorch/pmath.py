...
def _dist_for_vqvae(x, y, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    # print(_mobius_add(-x, y, c).shape)
    # print(_mobius_add(-x, y, c).norm(dim=1, p=2, keepdim=keepdim).shape)
    dist_c = artanh(sqrt_c * _mobius_add(-x, y, c).norm(dim=1, p=2, keepdim=keepdim))
    return dist_c * 2 / sqrt_c
